...
This guide covers **how to diff two profiling runs** so you can see per-function flat and cumulative time changes, optional HTML/JSON reports, and **optional CI-style failure** when regressions exceed a threshold.
...
| ---- | ---- | --------- | ------- | ----------- |
| `--base` | string | Yes | — | Baseline tag name (`bench/<base>/`). |
| `--current` | string | Yes | — | Current tag name. |
| `--bench-name` | string | Yes | — | Benchmark name (must match collect). |
| `--profile-type` | string | Yes | — | `cpu`, `memory`, `mutex`, or `block`. |
...
| Flag | Type | Required | Default | Description |
| ---- | ---- | --------- | ------- | ----------- |
| `--base` | string | Yes | — | Filesystem path to baseline profile. |
| `--current` | string | Yes | — | Filesystem path to current profile. |
| `--output-format` | string | Yes | — | Report format ([CLI reference](cli-reference.md#compare-output-formats)). |
...
| Value | Description |
| ----- | ----------- |
| `summary` / `detailed` | Text to stdout |
| `summary-html` / `detailed-html` | HTML files |
| `summary-json` / `detailed-json` | JSON files |
