...
def analyze_profiles(tag: str, profile_types: List[str]):
    ...
    benchmark_names = (validate_benchmark_directories(tag) if config.ai_config.all_benchmarks else config.ai_config.specific_benchmarks)
    selected_profile_types = (profile_types if config.ai_config.all_profiles else config.ai_config.specific_profiles)
